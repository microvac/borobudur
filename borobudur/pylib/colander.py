...
def Function(function, message=_('Invalid value')):
    ...
    def __call__(node, value):
        ...
        if not result:
            raise Invalid(node, translationstring.TranslationString(message, mapping={'val':value}))
...
def Range(min=None, max=None, min_err=None, max_err=None):
    ...
    def __call__(node, value):
        if min is not None:
            if value < min:
                raise Invalid(node, _(min_err, mapping={'val':value, 'min':min}))

        if max is not None:
            if value > max:
                raise Invalid(node, _(max_err, mapping={'val':value, 'max':max}))
    return __call__
...
class SchemaNode(object):
    ...
    def __init__(self, typ, *children, **kw):
        JS("@{{self}}._order = _schema_counter++")
        self.typ = typ
